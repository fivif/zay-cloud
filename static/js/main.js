...
function renderItems(items, categories) {
    ...
    filteredItems.forEach(item => {
        ...
        if (isCardView) {
            ...
            if (item.type === 'file') {
                    ...
                    previewHtml = `
                        <div class="file-preview" onclick="${itemClickHandler}">
                            ...
                            <img src="/view/${path}" alt="${item.name}" loading="lazy"
                                 onload="this.parentNode.querySelector('.loading-indicator').style.display='none';"
                                 onerror="this.onerror=null; this.src=''; this.parentNode.innerHTML='<i class=\'fas fa-image fa-3x\' style=\'color: rgba(0, 0, 0, 0.4);\'></i>';">
                        </div>`;
                } else if (item.media_type === 'video') {
                    // 视频文件显示截图预览
                    previewHtml = `
                ...
                <div class="file-preview" onclick="${itemClickHandler}">
                    ...
                    <div class="file-icon-fallback" style="display:none;">
                        <i class="fas fa-music fa-3x" style="color: rgba(0, 0, 0, 0.4);"></i>
                    </div>
                    <img src="/api/thumbnail/audio/${path}" alt="${item.name}" loading="lazy"
                        onload="this.parentNode.querySelector('.loading-indicator').style.display='none';"
                        onerror="this.parentNode.querySelector('.loading-indicator').style.display='none'; this.style.display='none'; this.parentNode.querySelector('.file-icon-fallback').style.display='flex';">
                </div>`;
                } else {
                    // 其他类型文件显示图标
            ...
            div.innerHTML = `
            ${previewHtml}
            <div class="file-name text-truncate" title="${item.name || "未命名"}">${item.name || "未命名"}</div>
            <div class="file-info" style="margin-bottom: 30px;">
                ${item.size !== undefined ? `<small class="text-muted">${formatFileSize(item.size)}</small>` : ''}
            </div>
            <div class="file-actions" style="bottom: 15px;">
                ${item.type === "file" ? `
                    ...
                    <button class="btn btn-sm btn-info me-1" onclick="event.stopPropagation(); generateDirectLink('${path}')" title="获取链接"><i class="fas fa-link"></i></button>
                    <button class="btn btn-sm btn-secondary me-1" onclick="event.stopPropagation(); showFileQRCode('${path}')" title="显示二维码"><i class="fas fa-qrcode"></i></button>
                ` : ""}
                <button class="btn btn-sm btn-danger" onclick="event.stopPropagation(); ${item.type === 'folder' ? `deleteFolder('${path}')` : `deleteFile('${path}')`}" title="删除"><i class="fas fa-trash"></i></button>
            </div>`;
    } else {
        // 列表视图模式
        div.innerHTML = `
        ...
        // 重置并刷新预加载观察者
        setTimeout(() => {
            preloadManager.refresh();
            // 立即加载所有项目，而不是等待滚动
            preloadManager.preloadAllItems();
        }, 100);
}
...
// 添加页面右下角的标签，链接到GitHub
document.addEventListener("DOMContentLoaded", () => {
    // 创建一个固定在右下角的div元素
    const footerTag = document.createElement('div');
    footerTag.innerHTML = `<a href="https://github.com/fivif/zay-cloud" target="_blank" rel="noopener noreferrer">power by zay</a>`;
    
    // 设置样式
    Object.assign(footerTag.style, {
        position: 'fixed',
        bottom: '10px',
        right: '10px',
        padding: '5px 10px',
    ...
    // 设置链接样式
    const link = footerTag.querySelector('a');
...
document.addEventListener("DOMContentLoaded", () => {
    // 定义左上角区域
    const cornerSize = 50; // 左上角区域大小（像素）
    let isHidden = false;
    
    // 检测鼠标位置并控制内容显示
    document.addEventListener('mousemove', (e) => {
        const isInCorner = e.clientX <= cornerSize && e.clientY <= cornerSize;
        
        // 检查是否有模态窗口显示
        const isModalVisible = document.querySelector('.modal.show') !== null || 
        ...
        // 正常的隐藏/显示逻辑
        if (isInCorner && !isHidden) {
            document.body.classList.add('hide-content');
            isHidden = true;
        } else if (!isInCorner && isHidden) {
            document.body.classList.remove('hide-content');
            isHidden = false;
        }
    });
    ...
    styleElement.textContent = `
        .hide-content *:not(body):not(html) {
            opacity: 0 !important;
            pointer-events: none !important;
            transition: opacity 0.3s ease;
        }
        
        .hide-content .github-corner,
        .hide-content .corner-hint,
        .hide-content header,
        .hide-content nav,
        .hide-content .breadcrumb-container,
        .hide-content #notification,
        .hide-content .content > *,
        .hide-content .modal,
        .hide-content .modal-backdrop,
        .hide-content .file-item,
        .hide-content #uploadProgressButton,
        .hide-content footer {
...
const preloadManager = {
    observer: null,
    scrollDirection: 'down', // 默认向下滚动
    lastScrollTop: 0,
    visibleItems: new Set(),
    inViewportItems: new Set(),
    ...
    init: function() {
        ...
        if (container) {
            ...
            // 添加滚轮事件监听
            container.addEventListener('wheel', this.handleWheelScroll.bind(this), { passive: true });
    ...
    handleScroll: function(e) {
        ...
        // 如果是滚轮引起的滚动，使用增强的预加载逻辑
        if (this.wheelScrolling) {
            // 滚轮滚动中，已经在wheel事件中处理了预加载
            return;
        }
        
        // 常规滚动（如拖动滚动条）时触发预加载检查
    ...
    handleWheelScroll: function(e) {
        ...
        this.wheelScrollSpeed = delta;
        this.lastWheelEvent = now;
        
        // 根据滚动速度调整预加载范围
        const preloadFactor = Math.min(Math.max(1, delta / 100), 10); // 限制在1-10之间
        const direction = e.deltaY > 0 ? 'down' : 'up';
        
        // 立即触发扩展预加载
        this.preloadForWheelScroll(direction, preloadFactor);
        ...
        }, 500); // 滚轮停止后500ms认为滚动结束
    },
    // 滚轮滚动优化的预加载方法
    preloadForWheelScroll: function(direction, factor = 1) {
        // console.log(`执行滚轮优化预加载，方向: ${direction}, 因子: ${factor}`);
        
        const items = Array.from(document.querySelectorAll('.file-item'));
        if (items.length === 0) return;
        
        // 动态计算预加载数量，根据滚动速度调整
        const baseCount = 10; // 基础预加载数量
        const preloadCount = Math.ceil(baseCount * factor);
        
        // 找到当前视口中第一个和最后一个可见元素
        let visibleItems = items.filter(item => {
            return this.inViewportItems.has(item.dataset.itemId);
        });
        
        if (visibleItems.length === 0) {
            // 如果没有可见项目，查找距离视口最近的项目
            const container = document.querySelector('.content');
        ...
        if (direction === 'down') {
            ...
            // console.log(`滚轮向下预加载: 从${startIndex}到${endIndex - 1}`);
            
            for (let i = startIndex; i < endIndex; i++) {
                if (i >= 0 && i < items.length) {
                    this.loadItemResources(items[i], 'high');
                }
            }
        } else {
            // 向上滚动，预加载上方的项目
            const endIndex = firstVisibleIndex - 1;
            const startIndex = Math.max(endIndex - preloadCount, 0);
            
            // console.log(`滚轮向上预加载: 从${startIndex}到${endIndex}`);
        ...
        this.observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting) {
                    // 元素进入视口
                    this.inViewportItems.add(itemId);
                    this.visibleItems.add(itemId);
                    
                    // 加载当前可见项的资源
                    this.loadItemResources(entry.target, 'high');
                    
                    // 检查是否需要预加载
                    const container = document.querySelector('.content');
                    if (container && 
                        (container.scrollHeight - container.scrollTop - container.clientHeight < this.preloadDistance)) {
                        // 如果是滚轮滚动，使用更大的预加载量
                        const preloadCount = this.wheelScrolling ? 15 : 5;
                        this.preloadNextBatch(preloadCount);
                    }
                } else {
                    // 元素离开视口
                    this.inViewportItems.delete(itemId);
            ...
            rootMargin: '300px 0px 300px 0px', // 增加到300px的检测范围
            threshold: 0.01
        });
    ...
    // 观察所有文件项
    observeAllItems: function() {
        const items = document.querySelectorAll('.file-item');
        items.forEach((item, index) => {
            // 添加唯一标识
            if (!item.dataset.itemId) {
                item.dataset.itemId = `item-${Date.now()}-${index}`;
            }
    ...
    // 检查是否需要预加载
    checkForPreload: function() {
        if (this.scrollDirection === 'down') {
            this.preloadNextBatch();
        } else {
            this.preloadPreviousBatch();
        }
    },
    
    // 预加载下一批文件
    preloadNextBatch: function(count = 5) {
        const items = Array.from(document.querySelectorAll('.file-item'));
        ...
        for (let i = 0; i < items.length; i++) {
            if (this.inViewportItems.has(items[i].dataset.itemId)) {
        ...
        // 预加载后面的元素
        const preloadCount = count;
        for (let i = 1; i <= preloadCount; i++) {
            const index = lastVisibleIndex + i;
            if (index < items.length) {
                this.loadItemResources(items[index], 'high');
            }
        }
    },
    // 预加载前一批文件
    preloadPreviousBatch: function(count = 5) {
        const items = Array.from(document.querySelectorAll('.file-item'));
    ...
    // 加载项目中的资源
// 加载项目中的资源
loadItemResources: function(item, priority) {
    if (!item) return;
    
    // 查找项目中的所有图片（包括eager和lazy加载的）
    const images = item.querySelectorAll('img[data-original-src]');
    images.forEach(img => {
        // 如果图片尚未加载，开始加载
        if (!img.src || img.src === window.location.href || img.src === "") {
            ...
            if (originalSrc) {
                // 先添加加载完成的事件监听
                img.onload = function() {
                    const loadingIndicator = img.parentNode.querySelector('.loading-indicator');
                    if (loadingIndicator) {
                ...
                img.onerror = function() {
                    ...
                    const fallback = img.parentNode.querySelector('.file-icon-fallback');
                    if (fallback) {
                        fallback.style.display = 'flex';
                    } else {
                        img.parentNode.innerHTML = '<i class="fas fa-image fa-3x" style="color: rgba(0, 0, 0, 0.4);"></i>';
                    }
                };
        ...
        // 预加载视频或音频缩略图
        const thumbnailContainers = item.querySelectorAll('[data-thumbnail-path]');
        thumbnailContainers.forEach(container => {
            const thumbnailPath = container.dataset.thumbnailPath;
            if (thumbnailPath && !container.dataset.preloaded) {
                ...
                preloader.onerror = function() {
                    container.dataset.preloaded = 'error';
                    const loadingIndicator = container.querySelector('.loading-indicator');
                    if (loadingIndicator) {
                        loadingIndicator.style.display = 'none';
                    }
                    ...
                    // 添加备用图标
                    container.innerHTML = '<i class="fas fa-video fa-3x" style="color: rgba(0, 0, 0, 0.4);"></i>';
                };
    ...
    // 添加一个新方法，用于立即预加载所有项目
    preloadAllItems: function() {
        // // console.log('开始预加载所有文件列表项...');
        const items = document.querySelectorAll('.file-item');
        
        if (items.length === 0) {
            // console.log('没有文件项可预加载');
            return;
        }
        
        // // console.log(`找到 ${items.length} 个文件项待预加载`);
        
        // 使用批次处理避免阻塞主线程
        const batchSize = 10; // 每批处理的项目数量
        const totalBatches = Math.ceil(items.length / batchSize);
        
        const processBatch = (batchIndex) => {
            if (batchIndex >= totalBatches) {
                // // console.log('所有文件项预加载完成');
                return;
            }
            ...
            // // console.log(`预加载批次 ${batchIndex + 1}/${totalBatches} (项目 ${start} 到 ${end - 1})`);
            ...
            for (let i = start; i < end; i++) {
                ...
                // 添加唯一标识（如果没有）
                if (!item.dataset.itemId) {
                    item.dataset.itemId = `item-${Date.now()}-${i}`;
                }
                // 以高优先级加载资源
                this.loadItemResources(item, 'high');
                // 将项目添加到已处理集合
                this.visibleItems.add(item.dataset.itemId);
            }
...
// 检查元素是否即将进入视口（用于手动触发预加载）
function isElementNearViewport(el, distance = 300) {
    if (!el) return false;
    ...
    if (vertInView && horInView) return true;
    
    // 检查元素是否在视口附近
    const topNear = rect.top - windowHeight <= distance && rect.top >= 0;
    const bottomNear = rect.bottom >= -distance && rect.bottom <= 0;
    
    return (topNear || bottomNear) && horInView;
}
...
function preloadVisibleAndNearby() {
    ...
    items.forEach(item => {
        if (isElementNearViewport(item, 500)) {
            preloadManager.loadItemResources(item, 'high');
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (container) {
        container.addEventListener('scroll', function() {
            requestAnimationFrame(preloadVisibleAndNearby);
        });
        
        // 初始预加载
        setTimeout(preloadVisibleAndNearby, 500);
    }
});
